...
public class AdminService {
    ...
    public DashboardStatsResponse getDashboardStats() {
        ...
        List<PopularPaintingDTO> bestSellingPaintings = paintingRepository.findBestSellingPaintings(top5).stream()
                .map(this::toPopularPaintingDTO)
                .collect(Collectors.toList());
    ...
    private PopularPaintingDTO toPopularPaintingDTO(Painting painting) {
        String primaryImage = painting.getImages().stream()
                ...
                .map(img -> ServletUriComponentsBuilder.fromCurrentContextPath()
                        ...
                        .toUriString())
                .orElseGet(() -> painting.getImages().isEmpty() ? null : ServletUriComponentsBuilder.fromCurrentContextPath()
                        .path("/api/images/")
                        .path(String.valueOf(painting.getImages().get(0).getId()))
                        .toUriString());
